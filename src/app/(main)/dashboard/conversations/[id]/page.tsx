...
import { AlertCircle, ArrowLeft, Bot, Check, Loader2, User, UserCheck } from "lucide-react";
...
export default function ConversationDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const loadConversation = async () => {
      ...
      if (result.conversation) {
        ...
        // If conversation belongs to a different workspace, redirect to conversations list
        if (result.error.includes("different workspace")) {
  ...
  async function handleReply() {
    ...
    const { replyToConversation } = await import("@/lib/auth/actions/conversation/conversation.actions");
    const result = await replyToConversation({ conversationId: id, content: reply });
      ...
      <div className="rounded-lg border">
        <div className="max-h-[500px] space-y-4 overflow-y-auto p-4">
          {messages.length === 0 ? (
            ...
            messages.map((msg) => {
              const isHumanMessage = msg.role === "assistant" && msg.metadata?.sender === "human";
              return (
                <div key={msg.id} className={`flex gap-2 ${msg.role === "user" ? "justify-end" : ""}`}>
                  ...
                  <div
                    className={`max-w-[75%] rounded-lg px-3 py-2 text-sm ${
                      msg.role === "user" ? "bg-primary text-primary-foreground" : isHumanMessage ? "border bg-blue-50" : "border bg-background"
                    }`}
                  >
        ...
        {conversation.status !== "resolved" && (
          <div className="border-t p-4">
            <div className="flex gap-2">
              <textarea
                placeholder={conversation.status === "human_required" ? "Waiting for human to join..." : "Type your reply..."}
                value={reply}
                ...
                disabled={isSending || conversation.status === "human_required"}
                ...
                maxLength={4000}
              />
              <Button onClick={handleReply} disabled={isSending || !reply.trim() || conversation.status === "human_required"}>
                {isSending ? <Loader2 className="size-4 animate-spin" /> : "Send"}
              </Button>
            </div>
            {conversation.status === "human_required" && (
              <p className="mt-2 text-muted-foreground text-xs">This conversation is waiting for a human to join.</p>
            )}
            {conversation.status === "with_human" && (
              <p className="mt-2 text-muted-foreground text-xs">You are handling this conversation. Type your reply above.</p>
            )}
          </div>
        )}
      </div>
