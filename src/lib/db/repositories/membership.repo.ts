import { getSupabaseAllMemberships, getSupabaseUserRole } from "@/lib/auth/supabase-identity";
...
export async function getUserRole(workspaceId: string, userId: string): Promise<string | null> {
  if (isSupabase()) {
    return getSupabaseUserRole(workspaceId, userId);
  }
...
/** Activates an invited membership, linking it to the real Directus user id. */
...
export async function getAllMemberships(): Promise<MembershipEntity[]> {
  if (isSupabase()) {
    return getSupabaseAllMemberships();
  }
