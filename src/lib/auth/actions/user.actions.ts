...
export interface CurrentUserResult {
  user: { id: string; email: string; name: string | null; avatar: string | null; forcePasswordReset: boolean };
...
export async function getCurrentUser(): Promise<CurrentUserResult> {
  ...
  if (!isAuthenticated || !user) {
    return {
      user: { id: "", email: "", name: null, avatar: null, forcePasswordReset: false },
      workspaces: [],
