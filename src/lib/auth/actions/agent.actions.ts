...
/**
 * Resolve the base system prompt for a newly created agent:
 * custom instructions > platform default (super admin) > generated default.
 */
export async function resolveSystemPrompt(instructions: string | undefined, name: string, tone: string): Promise<string> {
  if (instructions?.trim()) return instructions.trim();
  const defaults = await getAIDefaults();
  if (defaults.defaultSystemPrompt?.trim()) return defaults.defaultSystemPrompt.trim();
  return defaultSystemPrompt(name, tone);
}
  ...
  try {
    ...
    if (isLocalDev) {
      ...
      agent = localDb.agent.create({
        ...
        avatar: null,
        system_prompt: await resolveSystemPrompt(parsed.data.systemInstructions, parsed.data.name, parsed.data.tone),
      ...
      agent = await agentRepo.createAgent({
        ...
        language: parsed.data.language,
        systemPrompt: await resolveSystemPrompt(parsed.data.systemInstructions, parsed.data.name, parsed.data.tone),
        purpose: parsed.data.purpose,
...
export async function testAgentChat(data: {
  agentId: string;
  message: string;
  ...
  try {
    ...
    const { ragQuery } = await import("@/lib/ai/rag-pipeline");
    ...
    return { success: true, content: result.content, sources: result.sources, confidence: result.confidence };
  } catch (error) {
    const errMessage = error instanceof Error ? error.message : "";
    if (errMessage.startsWith("Unauthorized") || errMessage.startsWith("Forbidden")) {
