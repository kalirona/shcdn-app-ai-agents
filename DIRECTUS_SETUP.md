...
1. Reads the Directus session to get the user (`id`, `email`, `name`).
2. Calls `getUserWorkspaces(user.id)` -> queries `memberships` for rows where `user = <directus user id>`.
3. If none found, creates:
   - a `workspaces` row named `"<Name>'s Workspace"` with a unique slug, and
   - a `memberships` row with `user = <directus user id>`, `workspace = <new id>`, `role = "owner"`.
4. Returns the workspace list so the dashboard shows it.

So after John registers in Directus and lands on the dashboard, his workspace + owner
